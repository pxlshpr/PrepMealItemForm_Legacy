...
extension FormSize {
     ...
     */
    func unitWeightQuantity(in food: Food) -> FormQuantity? {
...
extension Food {
    ...
    var servingWeightQuantity: FormQuantity? {
        ...
        switch servingUnit {
        case .weight(let weightUnit):
            return FormQuantity(amount: serving.value, unit: .weight(weightUnit))
        case .size(let sizeUnit, let sizeVolumePrefix):
            guard let sizeUnitWeightQuantity = sizeUnit.unitWeightQuantity(in: self) else {
                return nil
            }
            return FormQuantity(amount: sizeUnitWeightQuantity.amount * serving.value, unit: sizeUnitWeightQuantity.unit)
        default:
            return nil
        }
    }
    
    func convert(_ quantity: FormQuantity, to unit: FormUnit) -> FormQuantity? {
        ...
        return FormQuantity(amount: converted, unit: unit)
    }
